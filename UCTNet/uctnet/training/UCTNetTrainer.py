...
from uctnet.network_architecture.uctnet_3D import UCTNet_3D
from uctnet.network_architecture.uctnet_2D import UCTNet_2D
...
from uctnet.training.utils import set_weight_decay

class UCTNetTrainer(nnUNetTrainer):
    """
    ...
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
                 ...
                 custom_network=None, task_id=None, count_params=False, max_num_epochs=1000):
    ...
    def initialize(self, training=True, force_load_plans=False):
        ...
        if not self.was_initialized:
            ...
            if self.deep_supervision:
                ...
                if self.task_id == 100:   #ISIC
                    # we don't use the lowest 1 outputs. Normalize weights so that they sum to 1
                    mask = np.array([True] + [True if i < net_numpool else False for i in range(1, net_numpool)])
                    weights[~mask] = 0
                    weights = weights / weights.sum()
                    weight_T = [3, 1, 1.5, 0.5, 0.1]
                else:
                    ...
                    weights[4] = weights[3]
                    if self.task_id == 17:
                        weight_T = [1, 1, 0.8, 0.5, 0.1]
                    elif self.task_id == 27:
                        weight_T = [3, 1, 2, 0.5, 0.1]
                    else:
                        weight_T = [1, 1, 0.8, 0.5, 0.1]
                
                self.ds_loss_weights = weights
                self.dsT_loss_weights = weight_T
                # now wrap the loss
                self.loss = DC_and_CE_loss({'batch_dice': self.batch_dice, 'smooth': 1e-5, 'do_bg': False}, {}) 
                self.loss = MultipleOutputLoss2(self.loss, self.ds_loss_weights, self.dsT_loss_weights)
                ################# END ###################
    ...
    def initialize_network(self):
        ...
        elif self.custom_network == "UCTNet_2D" and self.task_id == 27:
            self.network = UCTNet_2D(img_size=self.patch_size,
                                    ...
                                    patch_size=[[16,16],[8,8],[4,4],[2,2],[1,1],[1,1]], 
                                    dim_head=64,
                                    add_Map=True)
        elif self.custom_network == "UCTNet_2D" and self.task_id == 100:
            self.network = UCTNet_2D(img_size=self.patch_size,
                                    base_num_features=24,
                                    num_classes=self.num_classes,
                                    num_pool=len(self.net_num_pool_op_kernel_sizes),
                                    ...
                                    dim_head=64,
                                    add_Map=True)
        elif self.custom_network == "UCTNet_3D" and self.task_id == 17:
            self.network = UCTNet_3D(img_size=self.patch_size,
                                    base_num_features=24,
                                    num_classes=self.num_classes,
                                    num_pool=len(self.net_num_pool_op_kernel_sizes),
                                    image_channels=self.num_input_channels,
    ...
    def initialize_optimizer_and_scheduler(self):
        ...
        # self.optimizer = torch.optim.AdamW(parameters, self.initial_lr, weight_decay=self.weight_decay,
        #                                   amsgrad=True)
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False, is_infer=True):
        """
        ...
        if self.fp16:
            with autocast():
                if is_infer:
                    output, DS_T, UMap = self.network(data, target, is_infer)
                else:
                    output, DS_T, UMap = self.network(data, target, is_infer)
                
                del data
                l = self.loss(output, target, DS_T, UMap)
        ...
        while self.epoch < self.max_num_epochs:
            ...
            if self.use_progress_bar:
                with trange(self.num_batches_per_epoch) as tbar:
                    for b in tbar:
                        ...
                        l = self.run_iteration(self.tr_gen, True, False, False)
            ...
            with torch.no_grad():
                ...
                for b in range(self.num_val_batches_per_epoch):
                    l = self.run_iteration(self.val_gen, False, True, False)
                    val_losses.append(l)
                ...
                if self.also_val_in_tr_mode:
                    ...
                    val_losses = []
                    for b in range(self.num_val_batches_per_epoch):
                        l, _, _, _, _ = self.run_iteration(self.val_gen, False)
                        val_losses.append(l)
