...
class MYTrans_m2_2D(SegmentationNetwork):
    def __init__(self, img_size, base_num_features, num_classes, image_channels=1, num_only_conv_stage=2, num_conv_per_stage=2,
                 ...
                 conv_kernel_sizes=None, deep_supervision=True, max_num_features=None, dropout_p=0.1, use_checkpoint=False, **kwargs):
        super().__init__()
        self.num_classes = num_classes
    ...
    def forward(self, x, tar, is_infer):#
        ...
        if self.do_ds:
            ...
            return out[-1], out[-1], out[-1], out[-1][:,0]
    @torch.jit.ignore
