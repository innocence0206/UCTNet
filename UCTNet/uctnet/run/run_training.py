...
from uctnet.inference.metrics import get_dice_hd95
import uctnet
...
def main():
    ...
    parser.add_argument("-network_trainer", type=str, default='UCTNetTrainer')
    ...
    parser.add_argument("-ei", "--experiment_id", default="UCTNet")
    parser.add_argument("-eei", "--exist_experiment_id", required=False, default=None)
    parser.add_argument("--epoch_num", required=False, default=1000)
    ...
    task = args.task
    fold = args.fold
    max_num_epochs = int(args.epoch_num)
    ...
    plans_file, output_folder_name, dataset_directory, batch_dice, stage, \
        trainer_class = get_default_configuration(network, task, network_trainer, plans_identifier,
                                                  search_in=(uctnet.__path__[0], "training"),
                                                  base_module='uctnet.training') 
    trainer = trainer_class(plans_file, fold, output_folder=output_folder_name, dataset_directory=dataset_directory,
                            batch_dice=batch_dice, stage=stage, unpack_data=decompress_data, deterministic=deterministic,
                            fp16=run_mixed_precision, custom_batch_size=args.custom_batch_size, custom_network=args.custom_network,
                            experiment_id=experiment_id, task_id=task_id, max_num_epochs=max_num_epochs)
    ...
    if find_lr:
        ...
        if not validation_only:
            ...
            if valbest:
                ...
                chk = ""
                trainer.load_checkpoint(chk,train=False)
