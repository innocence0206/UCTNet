...
def get_default_configuration(network, task, network_trainer, plans_identifier=default_plans_identifier,
    ...
    # if network == '2d' and task == 'Task100_ISIC2018':
    #     plans['plans_per_stage'][0]['batch_size'] = 4
    #     plans['plans_per_stage'][0]['num_pool_per_axis'] = [5, 5]
    #     plans['plans_per_stage'][0]['pool_op_kernel_sizes'] = [[2, 2], [2, 2], [2, 2], [2, 2], [2, 2]]
    #     plans['plans_per_stage'][0]['conv_kernel_sizes'] = [[3, 3], [3, 3], [3, 3], [3, 3], [3, 3], [3, 3]]
    #     save_pickle(plans, join(preprocessing_output_dir, task, plans_identifier + "_plans_2D.pkl"))
